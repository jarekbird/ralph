...
# Usage: ./ralph.sh [max_iterations] [--worker amp|cursor] [--model MODEL] [--prd PRD_JSON]
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      if [[ "$1" =~ ^[0-9]+$ ]]; then
        MAX_ITERATIONS="$1"
      elif [[ -z "$PRD_FILE_OVERRIDE" && ( "$1" == *.json || "$1" == *.JSON ) ]]; then
...
PRD_FILE="${PRD_FILE_OVERRIDE:-$SCRIPT_DIR/prd.json}"
PRD_DIR="$(cd "$(dirname "$PRD_FILE")" 2>/dev/null && pwd || echo "$SCRIPT_DIR")"
